...
void vTask2_cmd_handling(void *params)
{
	...
	while(1)
	{
		...
		new_cmd = (APP_CMD_t*) pvPortMalloc(sizeof(APP_CMD_t));	// guardo dinamicamente la memoria para new_cmd

		taskENTER_CRITICAL(); 	//evito conflictos con variable command_buffer, recurso global compartido con ISR UART
...
void vTask3_cmd_processing(void *params)
{
	...
	while(1)
	{
		...
		vPortFree(new_cmd); //libero new_command alojado dinamicamente en vTask2_cmd_handling
	}
}

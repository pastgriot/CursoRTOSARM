...
void led_task_handler(void *params)
{
	...
	while(1)
	{
		...
		//espero hasta recibir el eventi de  notificacionn desde button task
		if(xTaskNotifyWait(0,0,&current_notif_value,portMAX_DELAY) == pdTRUE)
		{
			sprintf(usr_msg, "La Notificacion ha sido recibida : Conteo de boton presionado: %ld \r\n", current_notif_value );
			printmsg(usr_msg);

			//a falta de toggle..
			GPIO_WriteBit(GPIOE,GPIO_Pin_9, button_status_flag ^=1);
...
void button_task_handler(void *params){
	while(1)
	{
		...
		if (GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_0))
		{
			...
			//Se envia la notificación al task led
